...
Mobile.verifyElementText(findTestObject('Payment Screen/B1. Credit Card Payment/B1. OtherCreditCardPayment/OTP Screen - Card Holder Name'), 
    HolderName)

'Verify Pay From Account'
Mobile.verifyElementText(findTestObject('Payment Screen/B1. Credit Card Payment/B1. OtherCreditCardPayment/OTP Screen - Pay From'), 
    PayFrom, FailureHandling.CONTINUE_ON_FAILURE)

'Get Actual Amount showing on Device'
String actualPaymentAmount = Mobile.getText(findTestObject('Payment Screen/B1. Credit Card Payment/B1. OtherCreditCardPayment/OTP Screen - Amount'), 0).replaceAll('[a-zA-Z]+', '').trim()

'Verify Payment Amount'
Mobile.verifyEqual(actualPaymentAmount, PaymentAmount)


'Get grand Total from excel sheet'
String paymentAmn = ExcelKeywords.getCellValueByIndex(sheet01, 12, 5)

'Get Actual Amount showing on Device'
String actualGrandTotal = Mobile.getText(findTestObject('Payment Screen/B1. Credit Card Payment/B1. OtherCreditCardPayment/OTP Screen - Grand Total'), 0).replaceAll('[a-zA-Z]+', '').trim()

'Verify Grand Total Amount'
Mobile.verifyEqual(actualGrandTotal, paymentAmn)
...
Mobile.setText(findTestObject('Payment Screen/B1. Credit Card Payment/B1. OtherCreditCardPayment/OTP Screen - Enter OTP here'), 
    OtpNumber, 0)
